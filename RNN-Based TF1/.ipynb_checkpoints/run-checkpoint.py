...
def train_model(args):
	...
	for e in range(int(global_step/args.nbatches), args.nepochs):
		...
		for b in range(int(global_step%args.nbatches), args.nbatches):
			...
			#loss_dif_ = tf.Summary()
			#loss_dif_.value.add(tag='loss_dif',simple_value=valid_loss-train_loss)
			# train_loss_ = tf.Summary()
			# train_loss_.value.add(tag="train_loss", simple_value=train_loss)
			# summary_writer.add_summary(train_loss_)
			# valid_loss_ = tf.Summary()
			# valid_loss_.value.add(tag="valid_loss", simple_value=valid_loss)
			# summary_writer.add_summary(train_loss_)
			...
			# tf.summary.scalar('valid_loss', valid_loss_)



			#summary_writer_loss.add_summary(valid_loss_)
			#summary_writer.add_summary(loss_dif_)
			end = time.time()
...
def sample_model(args, logger=None):
	# if args.text == '':
	# 	strings = ['the i talked to duran the'] # test strings
	# else:
	# 	f=open('../enron_phrases_sample.txt','r')
	# 	i=0
	# 	strings=[]
	# 	while i <499990:
	# 		i+=1
...
#			s_save_path = '{}stroke/iter-{}-s-{}.txt'.format(args.log_dir, global_step, ss.replace(' ', '_'))
#
