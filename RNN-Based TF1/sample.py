...
def get_style_states(model, args):
    ...
    data_file = os.path.join(args.data_dir, "strokes_training_data.cpkl")
...
def sample(input_text, model, args):
    ...
    one_hot = [to_one_hot(input_text, model.ascii_steps, args.alphabet)]
    ss = to_string(input_text, model.ascii_steps, args.alphabet)
    ...
    prev_x = np.asarray([[[0, 0, 1]]], dtype=np.float32)     # start with a pen stroke
    ...
    # stroke=strokes[60:,:2]
    return strokes, phis, windows, kappas, ss
